...
The private-beta Visual Framework HUD is a compact **top-right Integrity Matrix**, not a centre-screen notification. Its original Aegis presentation combines a circular field-integrity focus with filled field and six-facing meters, heat/power diagnostics, current profile/mode, and an unobtrusive system rail. Field and facing fill transitions cyan → gold → red as protection depletes, while heat uses its own rising-risk route and power remains blue. The required Visual Framework includes the TROA-owned HUD service itself; no separate third-party HUD Workshop item is required. It is player-local and display-only: server authority remains with the shield runtime. The current beta repair also binds the HUD telemetry bridge to the confirmed live Visual Framework session so engine session-order differences cannot silently omit the panel.
