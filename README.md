...
The private-beta Visual Framework HUD is a full **top-right Integrity Matrix**, not a centre-screen notification. It is a graphical, no-text instrument panel: six directional shield wells, a direction ring, heat margin, thermal stabilization, atmosphere seal, suit-induction service, Jump Detection scan activity, and Bifrost transit readiness. The meter artwork remains stable while live mask composition reveals the available segments; change rails and indicator lights communicate rising/falling values without reading a text block. It refreshes from the live, read-only shield snapshot every 15 ticks, so slider and runtime changes update the graphical instruments. The layout preserves its authored 3:1 proportions on a 16:9 Space Engineers HUD and uses an uncompressed alpha panel to keep the fine instrumentation sharp. The required Visual Framework includes the TROA-owned HUD service itself; no separate third-party HUD Workshop item is required. It is player-local and display-only: server authority remains with the shield runtime. The HUD telemetry bridge is bound to the confirmed live Visual Framework session so engine session-order differences cannot silently omit the panel.
