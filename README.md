...
The private-beta Visual Framework HUD is a full **top-right Integrity Matrix**, not a centre-screen notification. Its graphical panel includes the authored live label hierarchy: six named directional shield wells and percentages, a direction ring, Heat Shields, Jump Detection, Thermal Stabilization, Atmosphere Seal, Suit Charge, and BI-FROST Transit readiness. It uses a real presentation pipeline: synchronized shield data is normalized into GUI state, displayed values transition using elapsed time, and only the supplied template’s defined regions are composited. No meter or value uses demonstration data or can alter gameplay. The Matrix refreshes from the live, read-only shield snapshot every 15 ticks, so shield damage, lifecycle changes, and applicable settings-driven runtime effects update both its bars and readouts. The layout preserves its authored 3:1 proportions on a 16:9 Space Engineers HUD and uses an uncompressed alpha panel to keep the fine instrumentation sharp. The required Visual Framework includes the TROA-owned HUD service itself; no separate third-party HUD Workshop item is required. It is player-local and display-only: server authority remains with the shield runtime. The HUD telemetry bridge is bound to the confirmed live Visual Framework session so engine session-order differences cannot silently omit the panel.
...
For private-beta testing, the required Visual Framework includes the Aegis-owned top-right Shield Health layout, colors, GUI asset, and HUD render service. It renders locally from a read-only snapshot refreshed four times per second. It never sends controls, changes a shield, or changes another player’s display. The normal game notification fallback is intentionally disabled because it cannot respect the requested screen position.
