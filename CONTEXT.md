...
- 2026-09-24: The private-beta exact-position Shield Health HUD uses the supported Text HUD API presentation surface. The required Visual Framework owns Aegis telemetry, layout, colors, and GUI asset while drawing player-local read-only data at the top-right. Shield authority remains exclusively in the server runtime and the central notification fallback is retired.
